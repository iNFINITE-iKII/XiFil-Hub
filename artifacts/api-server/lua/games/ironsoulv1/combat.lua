...
-- [FIX FIRST-RUN CRASH] Timeout 15s + pcall — kalau PlayerGui belum siap saat
-- cold start, jangan sampai hang/error dan mematikan sisa load chain.
do
    local playerGui = LocalPlayer:WaitForChild("PlayerGui", 15)
    if playerGui then
        local uiConn=playerGui.DescendantAdded:Connect(function(desc)
            task.wait(0.2); if isVictoryText(desc) then DisableAutoFarm("Victory Screen Detected") end
        end)
        RuntimeMaid:GiveTask(uiConn)
    else
        warn("[XiFil] PlayerGui timeout — listener Victory UI (jalur 1) dilewati")
    end
end
