...
local ThemeRegistry = {
    ...
    MultiCheckBtns={},   -- {btn, api} — pill button MultiCheckUI
    ParticleItems={},    -- {obj, colorProp, colorKey} — background effect particles
    MultiSelectRows={},  -- {btn, api} — scrollable multi-select row buttons
    SectionGradients={}, -- UIGradient refs for section header lines (follow Primary color)
    TooltipStrokes={},   -- UIStroke refs inside slider tooltips
    TooltipTexts={},     -- TextLabel refs inside slider tooltips
    TitleLabel = nil,    -- TextLabel ref for the top-bar title (rebuilt on each theme change)
}
local function RegisterThemeElement(cat, elem) table.insert(ThemeRegistry[cat], elem) end

-- Converts a Color3 to a CSS hex string (e.g. "#00dcff") for RichText <font color> tags.
...
-- Rebuilds the top-bar title RichText string with the current theme Primary color on "XIFIL".
local function RebuildTitleText(priHex)
    if not ThemeRegistry.TitleLabel then return end
    ThemeRegistry.TitleLabel.Text =
...
local function ApplyTheme()
    ...
    -- Simpan primary ke Hub agar modul lain bisa baca warna tema aktif
    H.CurrentThemePrimary = pri
    -- Rebuild title so "XIFIL" tracks the active theme colour
    RebuildTitleText(Color3ToHex(pri))
    ...
    -- MultiCheck pill buttons
    for _, entry in ipairs(ThemeRegistry.MultiCheckBtns) do
        pcall(function()
            entry.btn.BackgroundColor3 = entry.api.state and pri or Color3.fromRGB(35, 35, 50)
            entry.btn.TextColor3 = entry.api.state and Color3.fromRGB(10, 10, 15) or Color3.fromRGB(200, 200, 200)
        end)
    end
    -- Scrollable multi-select row buttons (active bg = versi gelap primary)
    local dimPri = Color3.new(pri.R * 0.28, pri.G * 0.28, pri.B * 0.28)
    for _, entry in ipairs(ThemeRegistry.MultiSelectRows) do
        pcall(function()
            entry.btn.BackgroundColor3 = entry.api.state and dimPri or Color3.fromRGB(22, 22, 35)
        end)
    end
    -- Particle frames / strokes
    for _, p in ipairs(ThemeRegistry.ParticleItems) do
        pcall(function() p.obj[p.colorProp] = GetThemeColor(p.colorKey) end)
    end
    -- Section header gradient lines (Primary → dark)
    for _, grad in ipairs(ThemeRegistry.SectionGradients) do
        pcall(function()
            grad.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, pri),
                ColorSequenceKeypoint.new(0.55, Color3.fromRGB(50,50,65)),
                ColorSequenceKeypoint.new(1,  Color3.fromRGB(15,15,22)),
            })
        end)
    end
    -- Slider tooltip strokes & texts
    for _, s in ipairs(ThemeRegistry.TooltipStrokes) do pcall(function() s.Color = pri end) end
    for _, t in ipairs(ThemeRegistry.TooltipTexts)  do pcall(function() t.TextColor3 = txt end) end
end
...
local function ClearBgEffect()
    ...
    ParticleTask = nil
    ThemeRegistry.ParticleItems = {}  -- clear stale particle refs
end
...
local function CreateSection(parent, titleText, langKey)
    ...
    lineGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,    GetThemeColor("Primary")),
        ColorSequenceKeypoint.new(0.55, Color3.fromRGB(50, 50, 65)),
        ColorSequenceKeypoint.new(1,    Color3.fromRGB(15, 15, 22)),
    })
    table.insert(ThemeRegistry.SectionGradients, lineGrad)
    -- Accent dot
    local dot = Instance.new("Frame", sec)
    dot.BackgroundColor3 = GetThemeColor("Primary")
    dot.Size = UDim2.new(0, 5, 0, 5); dot.AnchorPoint = Vector2.new(0, 0.5)
    dot.Position = UDim2.new(0, 8, 0.5, 0); dot.BorderSizePixel = 0
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
    RegisterThemeElement("Indicators", dot)
    -- Label
    local lbl = Instance.new("TextLabel", sec)
    lbl.BackgroundColor3 = Color3.fromRGB(15, 15, 22)
    lbl.Size = UDim2.new(0, 0, 1, 0); lbl.AutomaticSize = Enum.AutomaticSize.X
    lbl.Position = UDim2.new(0, 20, 0, 0); lbl.BorderSizePixel = 0
...
local function CreateToggleUI(parent, text, default, callback, langKey)
    ...
    -- Glow stroke: pulses when active
    local glowStroke = Instance.new("UIStroke", track)
    glowStroke.Color      = GetThemeColor("Primary")
    glowStroke.Thickness  = 2
    glowStroke.Transparency = default and 0.45 or 1
    RegisterThemeElement("Strokes", glowStroke)
    ...
    local thumbGrad = Instance.new("UIGradient", thumb)
    thumbGrad.Color    = ColorSequence.new({ ColorSequenceKeypoint.new(0, Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1, Color3.fromRGB(210,210,222)) })
    ...
    local function startGlowPulse()
        if glowTask then pcall(task.cancel, glowTask) end
        glowTask = task.spawn(function()
            while state do
                TweenService:Create(glowStroke, TweenInfo.new(0.85, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.05 }):Play()
                task.wait(0.85)
                if not state then break end
                TweenService:Create(glowStroke, TweenInfo.new(0.85, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.55 }):Play()
                task.wait(0.85)
            end
        end)
    end
    local function stopGlowPulse()
        if glowTask then pcall(task.cancel, glowTask); glowTask = nil end
        TweenService:Create(glowStroke, TweenInfo.new(0.2), { Transparency = 1 }):Play()
    end
    if default then startGlowPulse() end

    -- Hover on the whole row
    track.MouseEnter:Connect(function()
        TweenService:Create(container, TweenInfo.new(0.15), { BackgroundTransparency = 0.04 }):Play()
    end)
    track.MouseLeave:Connect(function()
        TweenService:Create(container, TweenInfo.new(0.15), { BackgroundTransparency = 0.2 }):Play()
    end)
...
local function CreateCycleUI(parent, text, list, default, callback)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(34, 34, 54) }):Play()
    end)
...
local function CreateButton(parent, text, callback, langKey)
    ...
    RegisterThemeElement("Strokes", bs)
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.18), { BackgroundColor3 = Color3.fromRGB(32, 32, 50) }):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.18), { BackgroundColor3 = Color3.fromRGB(22, 22, 35) }):Play()
    end)
    -- Click: press-down flash + ripple
    btn.MouseButton1Down:Connect(function(x, y)
        ...
        local relX = x - btn.AbsolutePosition.X
        local relY = y - btn.AbsolutePosition.Y
        ripple.Position = UDim2.fromOffset(relX, relY)
        ripple.BorderSizePixel = 0; ripple.ZIndex = btn.ZIndex + 2
        Instance.new("UICorner", ripple).CornerRadius = UDim.new(1, 0)
        TweenService:Create(ripple, TweenInfo.new(0.42, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
    ...
    btn.MouseButton1Up:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.14), { BackgroundTransparency = 0 }):Play()
    end)
...
local function CreateScrollableMultiSelectUI(parent, labelText, items, states, callbacks, langKey)
    ...
    table.insert(ThemeRegistry.AllLabels, lbl)
    if langKey and H.RegisterTranslation then H.RegisterTranslation(langKey, lbl, "Text") end
    ...
    local function _rowActiveBg()
        local p = H.CurrentThemePrimary or GetThemeColor("Primary")
        return Color3.new(p.R * 0.28, p.G * 0.28, p.B * 0.28)
    end

    for i, item in ipairs(items) do
        local rowBtn = Instance.new("TextButton", panel)
        rowBtn.BackgroundColor3 = currentStates[i] and _rowActiveBg() or Color3.fromRGB(22, 22, 35)
        ...
        local accentBar = Instance.new("Frame", rowBtn)
        accentBar.BackgroundColor3 = GetThemeColor("Primary")
        accentBar.Size = UDim2.new(0, 3, 1, -6); accentBar.Position = UDim2.new(0, 0, 0, 3)
        accentBar.BorderSizePixel = 0; accentBar.ZIndex = rowBtn.ZIndex + 1
        accentBar.Visible = currentStates[i]
        Instance.new("UICorner", accentBar).CornerRadius = UDim.new(0, 3)
        RegisterThemeElement("Indicators", accentBar)

        local api = { state = currentStates[i] }
        local function applyRowVisual(val)
            api.state = val; currentStates[i] = val
            TweenService:Create(rowBtn, TweenInfo.new(0.14), {
                BackgroundColor3 = val and _rowActiveBg() or Color3.fromRGB(22, 22, 35)
            }):Play()
            rowBtn.Text = "   " .. item
            accentBar.Visible = val
            updateSummary()
        end
        function api:SetValue(val)
            applyRowVisual(val)
        ...
        rowBtn.MouseButton1Click:Connect(function()
            -- Bounce click feedback
            TweenService:Create(rowBtn, TweenInfo.new(0.07), { BackgroundTransparency = 0.3 }):Play()
            task.delay(0.07, function()
                TweenService:Create(rowBtn, TweenInfo.new(0.12), { BackgroundTransparency = 0 }):Play()
            end)
        ...
        rowBtn.MouseLeave:Connect(function()
            TweenService:Create(rowBtn, TweenInfo.new(0.12), {
                BackgroundColor3 = api.state and _rowActiveBg() or Color3.fromRGB(22, 22, 35)
            }):Play()
        end)
        table.insert(ThemeRegistry.MultiSelectRows, { btn = rowBtn, api = api })
        apis[i] = api
    end
...
local function CreateMultiCheckUI(parent, labelText, items, states, callbacks)
    ...
    for i, item in ipairs(items) do
        ...
        table.insert(ThemeRegistry.MultiCheckBtns, {btn=btn, api=api})
        apis[i] = api
    end
    return apis
end
...
local function CreateSliderUI(parent, labelText, min, max, default, callback, langKey)
    ...
    track.Position = UDim2.new(0, 14, 0, 38); track.Size = UDim2.new(1, -28, 0, 6); track.BorderSizePixel = 0
    Instance.new("UICorner", track).CornerRadius = UDim.new(1, 0)
    -- Fill with gradient Primary → Secondary
    ...
    fillGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GetThemeColor("Primary")),
        ColorSequenceKeypoint.new(1, GetThemeColor("Secondary")),
    })
    ...
    thumb.Text = ""; thumb.AutoButtonColor = false; thumb.BorderSizePixel = 0; thumb.ZIndex = 3
    Instance.new("UICorner", thumb).CornerRadius = UDim.new(1, 0)
    local thumbGrad2 = Instance.new("UIGradient", thumb)
    thumbGrad2.Color    = ColorSequence.new({ ColorSequenceKeypoint.new(0, Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1, Color3.fromRGB(212,212,224)) })
    thumbGrad2.Rotation = 90
    -- Tooltip above thumb
    local tooltip = Instance.new("Frame", track)
    tooltip.BackgroundColor3 = Color3.fromRGB(22, 22, 38)
    tooltip.Size = UDim2.fromOffset(38, 22)
    tooltip.AnchorPoint = Vector2.new(0.5, 1)
    tooltip.Position = UDim2.new((default-min)/(max-min), 0, 0, -6)
    tooltip.BorderSizePixel = 0; tooltip.Visible = false; tooltip.ZIndex = 5
    Instance.new("UICorner", tooltip).CornerRadius = UDim.new(0, 5)
    local ttStroke = Instance.new("UIStroke", tooltip)
    ttStroke.Color = GetThemeColor("Primary"); ttStroke.Thickness = 1; ttStroke.Transparency = 0.35
    table.insert(ThemeRegistry.TooltipStrokes, ttStroke)
    local ttLabel = Instance.new("TextLabel", tooltip)
    ttLabel.BackgroundTransparency = 1; ttLabel.Size = UDim2.new(1, 0, 1, 0)
    ttLabel.Font = Enum.Font.GothamBold; ttLabel.Text = tostring(default)
    ttLabel.TextColor3 = GetThemeColor("Text"); ttLabel.TextSize = 10
    ttLabel.TextXAlignment = Enum.TextXAlignment.Center
    table.insert(ThemeRegistry.TooltipTexts, ttLabel)
    table.insert(ThemeRegistry.AllLabels, ttLabel)
    local dragging = false
    local function updateVisual(value)
        local alpha = math.clamp((value - min) / (max - min), 0, 1)
        fill.Size       = UDim2.new(alpha, 0, 1, 0)
        thumb.Position  = UDim2.new(alpha, -9, 0.5, -9)
        tooltip.Position = UDim2.new(alpha, 0, 0, -6)
        valLabel.Text   = tostring(value)
        ttLabel.Text    = tostring(value)
    end
    ...
    thumb.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            ...
            TweenService:Create(thumb, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                { BackgroundColor3 = Color3.fromRGB(230, 230, 245) }):Play()
        end
    end)
    ...
    UserInputService.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            ...
            TweenService:Create(thumb, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(255, 255, 255) }):Play()
        end
    end)
    local api = {}
...
-- Subtle transparency gradient: lighter at top, slightly darker toward bottom
...
Title.RichText = true
-- Initial text uses the current theme primary for "XIFIL"; rebuilt on every theme change via RebuildTitleText.
Title.Text =
    '<font size="15" color="'..Color3ToHex(GetThemeColor("Primary"))..'">XIFIL</font>'..
    '<font size="15" color="#ffffff"> HUB</font>'..
    '   <font size="15" color="#808099">//</font>   '..
    '<font size="7" color="#ffffff">IRON SOUL V6</font>'
...
-- Store ref so ApplyTheme can rebuild the rich text string on theme changes
ThemeRegistry.TitleLabel = Title
-- Subtle glow stroke on title
local titleStroke = Instance.new("UIStroke", Title)
titleStroke.Color       = GetThemeColor("Primary")
titleStroke.Thickness   = 1
titleStroke.Transparency = 0.62
RegisterThemeElement("Strokes", titleStroke)
RegisterThemeElement("Texts", Title); table.insert(ThemeRegistry.AllLabels, Title)

-- Window Control Buttons — macOS-style coloured circles
local WindowControls = Instance.new("Frame", TopBar)
WindowControls.BackgroundTransparency = 1
WindowControls.Position = UDim2.new(1, -90, 0, 0)
WindowControls.Size     = UDim2.new(0, 84, 1, 0)
WindowControls.ZIndex   = 5

-- Creates a single macOS-style circle button at the given X offset.
-- dotColor : resting circle colour
-- hoverIcon: single-char symbol that fades in on hover
local function CreateMacBtn(parent, xOff, dotColor, hoverIcon)
    local wrap = Instance.new("TextButton", parent)
    wrap.BackgroundTransparency = 1
    wrap.Position = UDim2.new(0, xOff, 0.5, 0)
    wrap.AnchorPoint = Vector2.new(0, 0.5)
    wrap.Size = UDim2.new(0, 22, 0, 22)
    wrap.Text = ""; wrap.AutoButtonColor = false; wrap.ZIndex = 6

    local circle = Instance.new("Frame", wrap)
    circle.BackgroundColor3 = dotColor
    circle.Size  = UDim2.new(0, 14, 0, 14)
    circle.Position = UDim2.new(0.5, 0, 0.5, 0)
    circle.AnchorPoint  = Vector2.new(0.5, 0.5)
    circle.BorderSizePixel = 0; circle.ZIndex = 6
    Instance.new("UICorner", circle).CornerRadius = UDim.new(1, 0)

    local icon = Instance.new("TextLabel", circle)
    icon.BackgroundTransparency = 1
    icon.Size = UDim2.new(1, 0, 1, 0)
    icon.Font = Enum.Font.GothamBold; icon.Text = hoverIcon
    icon.TextColor3 = Color3.fromRGB(40, 30, 30)
    icon.TextSize   = 8; icon.TextTransparency = 1
    icon.TextXAlignment = Enum.TextXAlignment.Center
    icon.ZIndex = 7

    wrap.MouseEnter:Connect(function()
        TweenService:Create(circle, TweenInfo.new(0.15), { Size = UDim2.new(0, 16, 0, 16) }):Play()
        TweenService:Create(icon,   TweenInfo.new(0.15), { TextTransparency = 0 }):Play()
    end)
    wrap.MouseLeave:Connect(function()
        TweenService:Create(circle, TweenInfo.new(0.15), { Size = UDim2.new(0, 14, 0, 14) }):Play()
        TweenService:Create(icon,   TweenInfo.new(0.15), { TextTransparency = 1 }):Play()
    end)
    wrap.MouseButton1Down:Connect(function()
        TweenService:Create(circle, TweenInfo.new(0.08), { BackgroundTransparency = 0.35 }):Play()
    end)
    wrap.MouseButton1Up:Connect(function()
        TweenService:Create(circle, TweenInfo.new(0.12), { BackgroundTransparency = 0 }):Play()
    end)
    return wrap
end

local BtnClose = CreateMacBtn(WindowControls,  4, Color3.fromRGB(255, 96,  88),  "×")
local BtnMin   = CreateMacBtn(WindowControls, 30, Color3.fromRGB(255, 189, 68),  "−")
local BtnMax   = CreateMacBtn(WindowControls, 56, Color3.fromRGB(40,  200, 64),  "+")
...
SwitchTab = function(tabName)
    ...
    for name, tdata in pairs(TabRegistry) do
        ...
        -- Pill: fade in when active, instant hide when inactive
        if isActive then
            tdata.Pill.Visible = true
            tdata.Pill.BackgroundTransparency = 1
            TweenService:Create(tdata.Pill, TweenInfo.new(0.22, Enum.EasingStyle.Quint), { BackgroundTransparency = 0 }):Play()
        else
            tdata.Pill.Visible = false
        end
...
local function CreateTab(tabName, langKey)
    ...
    -- Hover highlight on tab button
    tabBtn.MouseEnter:Connect(function()
        if ActiveTab ~= tabName then
            TweenService:Create(tabBtn, TweenInfo.new(0.15), { TextColor3 = Color3.fromRGB(195, 195, 215) }):Play()
        end
    end)
    tabBtn.MouseLeave:Connect(function()
        if ActiveTab ~= tabName then
            TweenService:Create(tabBtn, TweenInfo.new(0.15), { TextColor3 = Color3.fromRGB(140, 140, 165) }):Play()
        end
    end)
    ...
    Pill.AnchorPoint = Vector2.new(0.5, 1); Pill.ZIndex = 5; Pill.Visible = false
    ...
    sideBtn.TextSize = 10; sideBtn.AutoButtonColor = false; sideBtn.BorderSizePixel = 0
    Instance.new("UICorner", sideBtn).CornerRadius = UDim.new(0, 7)
    table.insert(ThemeRegistry.AllLabels, sideBtn)

    -- Hover highlight on side button
    sideBtn.MouseEnter:Connect(function()
        if ActiveTab ~= tabName then
            TweenService:Create(sideBtn, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(26, 26, 44) }):Play()
        end
    end)
    sideBtn.MouseLeave:Connect(function()
        if ActiveTab ~= tabName then
            TweenService:Create(sideBtn, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(22, 22, 36) }):Play()
        end
    end)
...
local function ApplyBgEffect(effectName)
    ...
    if effectName == "Ambient Aura" then
        ...
        for i = 1, 5 do
            ...
            table.insert(ThemeRegistry.ParticleItems, {obj=orb, colorProp="BackgroundColor3", colorKey=(i%2==0) and "Primary" or "Secondary"})
        end
    ...
    elseif effectName == "Nano Dust" then
        ...
        for i = 1, 35 do
            ...
            table.insert(ThemeRegistry.ParticleItems, {obj=d, colorProp="BackgroundColor3", colorKey="Primary"})
        end
        ParticleTask = task.spawn(function()
            while task.wait(0.03) do
                if not overlay.Parent then break end
    ...
    elseif effectName == "Fluid Waves" then
        ...
        for i = 1, 3 do
            ...
            table.insert(ThemeRegistry.ParticleItems, {obj=w, colorProp="BackgroundColor3", colorKey=i==1 and "Primary" or "Secondary"})
        end
        ParticleTask = task.spawn(function()
            while task.wait(0.03) do
                if not overlay.Parent then break end
    ...
    elseif effectName == "Glass Shards" then
        ...
        for i = 1, 8 do
            ...
            table.insert(ThemeRegistry.ParticleItems, {obj=stroke, colorProp="Color", colorKey="Primary"})
        end
        ParticleTask = task.spawn(function()
            while task.wait(0.03) do
                if not overlay.Parent then break end
...
local function ApplyAllVisuals()
    -- Setiap bagian dibungkus pcall sendiri agar satu error tidak menghentikan yang lain
    pcall(ApplyBgColorTheme, VisualConfig.CurrentBg)
    pcall(ApplyTheme)
    pcall(ApplyFont)
    pcall(ApplyToggleShape)
    pcall(ApplyButtonShape)
    ...
    pcall(function()
        local w = math.max(VisualConfig.GuiMinWidth  or 340, VisualConfig.GuiWidth  or 500)
        local h = math.max(VisualConfig.GuiMinHeight or 300, VisualConfig.GuiHeight or 440)
        MainWindow.Size     = UDim2.new(0, w, 0, h)
        -- Pusatkan ulang posisi window setelah ukuran berubah
        MainWindow.Position = UDim2.new(0.5, -w/2, 0.5, -h/2)
    end)
end
