...
-- Ini sengaja dibuat konsisten lintas perangkat: login dengan akun yang sama
-- di HP/PC manapun tetap dianggap "identitas" yang sama, sementara akun
-- Roblox yang berbeda selalu dianggap identitas yang berbeda meski di
-- device yang sama (lihat /setaccountlimit untuk atur berapa akun per key).
...
local function promptKey(callback)
    -- [FIX FIRST-RUN CRASH] Timeout 15s — kalau PlayerGui belum siap saat cold
    -- start, jangan hang selamanya di sini (fallback FindFirstChild).
...
--------------------------------------------------------------------------------
-- [FIX FIRST-RUN CRASH] Kalau script di-execute (autoexec/Xeno) sebelum game
-- benar-benar selesai loading, semua WaitForChild/GetService di modul-modul
-- di bawah bisa hang atau gagal secara acak. Tunggu game:IsLoaded() dulu
-- (dengan batas wajar 30 detik lewat game.Loaded:Wait() yang otomatis resolve
-- kalau sudah loaded) sebelum mulai load modul apapun.
--
-- [FIX XENO EXECUTOR] IsLoaded() saja tidak cukup: Xeno (dan beberapa executor
-- lain) menjalankan script SEBELUM game.Players.LocalPlayer ter-assign. Kalau
-- itu terjadi, promptKey() di bawah memanggil
-- game.Players.LocalPlayer:WaitForChild(...) selagi LocalPlayer masih nil →
...
startWithDRM(function(key, hwid)
    ...
    ----------------------------------------------------------------------------
    local BASE = "https://xifil-hub-production.up.railway.app/api/lua/module/ironsoulv1/"
