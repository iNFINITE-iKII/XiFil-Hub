...
local LANG_STRINGS = {
    Indonesia = {
        ...
        btnScanGoldShop="🔄 Scan Toko Emas",
    ...
    English = {
        ...
        tabSell="🏪 Sell",         tabRoom="🗺️ Room",         tabForge="⚒️ Forge",
...
local function ApplyFont()
    local font = FONT_MAP[VisualConfig.CurrentFont] or Enum.Font.GothamMedium
    for _, lbl in ipairs(ThemeRegistry.AllLabels) do pcall(function() lbl.Font = font end) end
end
...
local function CreateDropdownUI(parent, labelText, list, default, callback, langKey)
    ...
    container.BackgroundColor3 = Color3.fromRGB(20, 20, 30)
    container.BackgroundTransparency = 0.2
    container.Size = UDim2.new(1, 0, 0, 42); container.ClipsDescendants = false
    container.ZIndex = 5; container.BorderSizePixel = 0
    Instance.new("UICorner", container).CornerRadius = UDim.new(0, 8)

    local lbl = Instance.new("TextLabel", container)
    lbl.BackgroundTransparency = 1; lbl.Position = UDim2.new(0, 14, 0, 0)
    lbl.Size = UDim2.new(0.48, 0, 1, 0); lbl.Font = Enum.Font.GothamMedium
    lbl.Text = labelText; lbl.TextColor3 = Color3.fromRGB(205, 205, 218)
    lbl.TextSize = 12; lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 6
    table.insert(ThemeRegistry.AllLabels, lbl)
    if langKey then RegisterTranslation(langKey, lbl, "Text") end
    ...
    mainBtn.TextColor3 = GetThemeColor("Text"); mainBtn.TextSize = 11
    mainBtn.AutoButtonColor = false; mainBtn.ZIndex = 7; mainBtn.BorderSizePixel = 0
    ...
    panelClip.ClipsDescendants = true; panelClip.Visible = false
    panelClip.ZIndex = 199; panelClip.BorderSizePixel = 0

    local panel = Instance.new("ScrollingFrame", panelClip)
    panel.BackgroundColor3 = Color3.fromRGB(18, 18, 32)
    panel.Position = UDim2.new(0, 0, 0, 0)
    panel.Size = UDim2.new(1, 0, 1, 0)
    panel.ZIndex = 200; panel.ScrollBarThickness = 3
    panel.ScrollBarImageColor3 = GetThemeColor("Primary")
    panel.AutomaticCanvasSize = Enum.AutomaticSize.Y; panel.BorderSizePixel = 0
    ...
    local function refreshItems()
        ...
        for _, valName in ipairs(api.CurrentList) do
            ...
            ib.Size = UDim2.new(1, 0, 0, 30); ib.Font = Enum.Font.GothamMedium
            ib.Text = "  " .. tostring(valName); ib.TextColor3 = Color3.fromRGB(210, 210, 228)
            ib.TextSize = 11; ib.TextXAlignment = Enum.TextXAlignment.Left
            ib.ZIndex = 201; ib.BorderSizePixel = 0
            ...
            ib.MouseButton1Click:Connect(function()
                ...
                task.wait(0.2)
                panelClip.Visible = false; container.ZIndex = 5; isOpen = false; animating = false
    ...
    mainBtn.MouseButton1Click:Connect(function()
        ...
        if isOpen then
            panelClip.Visible = true; container.ZIndex = 100
            ...
            task.wait(0.25); panelClip.Visible = false; container.ZIndex = 5; animating = false
        end
    end)

    function api:SetValues(nl)
        api.CurrentList = nl; api.SelectedValue = nl[1] or "None"
        mainBtn.Text = tostring(api.SelectedValue) .. "  ▾"
        refreshItems()
    end
    function api:SetValue(tv)
        api.SelectedValue = tv; mainBtn.Text = tostring(tv) .. "  ▾"
        panelClip.Visible = false; container.ZIndex = 5; isOpen = false; animating = false
        pcall(callback, tv)
    end
...
MainWindow.Visible = false; MainWindow.BorderSizePixel = 0; MainWindow.ZIndex = 2
...
TopBar.Size = UDim2.new(1, 0, 0, 46); TopBar.BorderSizePixel = 0; TopBar.ZIndex = 3
Instance.new("UICorner", TopBar).CornerRadius = UDim.new(0, 12)
RegisterPanel(TopBar)

local TBFix = Instance.new("Frame", TopBar)
TBFix.BackgroundColor3 = Color3.fromRGB(18, 18, 30); TBFix.BorderSizePixel = 0
TBFix.Size = UDim2.new(1, 0, 0, 12); TBFix.Position = UDim2.new(0, 0, 1, -12); TBFix.ZIndex = 3

local TopAccent = Instance.new("Frame", TopBar)
TopAccent.BackgroundColor3 = GetThemeColor("Primary")
TopAccent.Size = UDim2.new(1, 0, 0, 1); TopAccent.Position = UDim2.new(0, 0, 1, -1)
TopAccent.BorderSizePixel = 0; TopAccent.BackgroundTransparency = 0.6; TopAccent.ZIndex = 4
RegisterThemeElement("Fills", { elem = TopAccent, _type = "primary" })
MakeDraggable(TopBar, MainWindow)

local Title = Instance.new("TextLabel", TopBar)
Title.BackgroundTransparency = 1; Title.Position = UDim2.new(0, 16, 0, 0)
Title.Size = UDim2.new(0.55, 0, 1, 0); Title.Font = Enum.Font.GothamBlack
Title.RichText = true
Title.Text = 'XIFIL <font color="#ffffff">HUB</font> <font size="10" color="#606080">// IRON SOUL V5</font>'
Title.TextColor3 = GetThemeColor("Text"); Title.TextSize = 15
...
WindowControls.Size = UDim2.new(0, 120, 1, 0); WindowControls.ZIndex = 5

local function CreateWinButton(parent, text, xPos)
    local btn = Instance.new("TextButton", parent)
    btn.BackgroundTransparency = 1
    btn.Position = UDim2.new(0, xPos, 0, 0); btn.Size = UDim2.new(0, 40, 1, 0)
    btn.Font = Enum.Font.GothamMedium; btn.Text = text
    btn.TextColor3 = Color3.fromRGB(200, 200, 200); btn.TextSize = 14
    btn.AutoButtonColor = false
    local highlight = Instance.new("Frame", btn)
    highlight.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    highlight.BackgroundTransparency = 1; highlight.Size = UDim2.new(1, 0, 1, 0); highlight.BorderSizePixel = 0
...
ResizeHandle.Text = ""; ResizeHandle.AutoButtonColor = false; ResizeHandle.ZIndex = 10; ResizeHandle.BorderSizePixel = 0
...
SideBar.Size = UDim2.new(0, 90, 1, -46); SideBar.BorderSizePixel = 0; SideBar.ZIndex = 3; SideBar.Visible = false
...
BtnMin.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        TweenService:Create(MainWindow, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = UDim2.new(0, MainWindow.Size.X.Offset, 0, 46)}):Play()
        ContentFrame.Visible = false; TabBar.Visible = false; SideBar.Visible = false; ResizeHandle.Visible = false
    else
        TweenService:Create(MainWindow, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = isWindowMaximized and UDim2.new(1,0,1,0) or normalSize}):Play()
...
BtnMax.MouseButton1Click:Connect(function()
    ...
    if isWindowMaximized then
        ...
        TweenService:Create(MainWindow, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 36), Position = UDim2.new(0, 0, 0, -36)}):Play()
        Instance.new("UICorner", MainWindow).CornerRadius = UDim.new(0, 0); ResizeHandle.Visible = false
    else
        TweenService:Create(MainWindow, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = normalSize, Position = normalPos}):Play()
        Instance.new("UICorner", MainWindow).CornerRadius = UDim.new(0, 12); ResizeHandle.Visible = true
    end
end)
...
--------------------------------------------------------------------------------
-- [S22] TAB 6 — AUTO BUY (hidden)
--------------------------------------------------------------------------------
local BuyPage = CreateTab("🛒 Buy", "tabBuy")
TabRegistry["🛒 Buy"].Button.Visible  = false
TabRegistry["🛒 Buy"].SideBtn.Visible = false
CreateSection(BuyPage, "Gold Shop Auto-Buyer", "secGoldShop")

local BuyButtonsRef = {}
local ShopListContainer = Instance.new("ScrollingFrame", BuyPage)
ShopListContainer.Name = "SLC"; ShopListContainer.Size = UDim2.new(1,0,0,220); ShopListContainer.BackgroundTransparency = 1
...
CreateButton(BuyPage, "🔄 Scan Gold Shop", function()
    ...
    for _, item in ipairs(sf:GetChildren()) do
        if item.Name:find("GoldShop") then
            total = total+1
            local btn = Instance.new("TextButton",ShopListContainer); btn.Size = UDim2.new(1,-10,0,32)
            btn.Font = Enum.Font.GothamMedium; btn.TextSize = 11; btn.TextXAlignment = Enum.TextXAlignment.Left; btn.BorderSizePixel = 0
            btn.BackgroundColor3 = EngineConfig.AutoBuyTargetList[item.Name] and Color3.fromRGB(40,90,50) or Color3.fromRGB(28,28,40)
            btn.TextColor3 = Color3.fromRGB(255,255,255); btn.Text = "  "..item.Name
            Instance.new("UICorner",btn).CornerRadius = UDim.new(0,6); BuyButtonsRef[item.Name] = btn
            btn.MouseButton1Click:Connect(function()
                if EngineConfig.AutoBuyTargetList[item.Name] then
                    EngineConfig.AutoBuyTargetList[item.Name] = nil; btn.BackgroundColor3 = Color3.fromRGB(28,28,40)
                else EngineConfig.AutoBuyTargetList[item.Name] = true; btn.BackgroundColor3 = Color3.fromRGB(40,90,50) end
            end, "btnScanGoldShop")
        end
...
CreateDropdownUI(AppearPage, "Background", BG_THEME_NAMES, VisualConfig.CurrentBg, function(v)
    VisualConfig.CurrentBg = v; ApplyBgColorTheme(v)
end, "lblBackground")

CreateSection(AppearPage, "Tema Warna GUI", "secTheme")
CreateDropdownUI(AppearPage, "🎨 Tema Warna", THEME_NAMES, VisualConfig.CurrentTheme, function(v)
...
CreateToggleUI(AppearPage, "🌫️ Mode Transparan", VisualConfig.TransparentMode, function(v)
    VisualConfig.TransparentMode = v; ApplyTransparency()
    CustomNotify("🌫️ TRANSPARAN", v and "Aktif" or "Nonaktif", 2)
end, "lblTranspMode")
CreateSliderUI(AppearPage, "🔆 Level Transparansi", 0, 100, math.floor(VisualConfig.TransparencyLevel * 100), function(v)
...
CreateDropdownUI(AppearPage, "🖐️ Mode Buka GUI", { "Classic", "Keybind [F]" }, VisualConfig.GestureMode, function(v)
    VisualConfig.GestureMode = v; CustomNotify("🖐️ GESTURE","Mode buka: "..v,2)
end, "lblGesture")

CreateSection(AppearPage, "Mode Tab", "secTabMode")
CreateDropdownUI(AppearPage, "📑 Orientasi Tab", { "Horizontal", "Vertikal" }, VisualConfig.TabMode, function(v)
    VisualConfig.TabMode = v; ApplyTabMode(v)
    CustomNotify("📑 TAB","Mode: "..v,2)
end, "lblTabMode")

CreateSection(AppearPage, "Bahasa / Translate", "secLang")
CreateDropdownUI(AppearPage, "🌐 Bahasa", LANGUAGES, CurrentLang, function(v)
...
CreateDropdownUI(FontPage, "🔤 Pilihan Font", FONT_LIST, VisualConfig.CurrentFont, function(v)
...
CreateDropdownUI(FontPage, "🔘 Bentuk Toggle", TOGGLE_SHAPES, VisualConfig.ToggleShape, function(v)
...
CreateDropdownUI(FontPage, "🟦 Bentuk Button", BUTTON_SHAPES, VisualConfig.ButtonShape, function(v)
...
CreateDropdownUI(AnimPage, "✨ Animasi", ANIM_STYLES, VisualConfig.AnimStyle, function(v)
    VisualConfig.AnimStyle = v; CustomNotify("✨ ANIMASI","Gaya animasi: "..v,2)
end, "lblAnimStyle")

CreateSection(AnimPage, "Efek Latar & Partikel", "secBgFx")
CreateDropdownUI(AnimPage, "🌟 Efek Latar", BG_EFFECTS, VisualConfig.BgEffect, function(v)
...
function SyncAllVisualUI()
    ...
    -- Terapkan semua setting visual setelah data game di-sync
...
local function ToggleGUI()
    ...
    if guiVisible then
        ...
        TweenService:Create(MainWindow, TweenInfo.new(0.35, easingStyle, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, VisualConfig.GuiWidth, 0, VisualConfig.GuiHeight),
            Position = UDim2.new(0.5, -VisualConfig.GuiWidth/2, 0.5, -VisualConfig.GuiHeight/2)
        }):Play()
    else
        ...
        task.wait(0.29); MainWindow.Visible = false
        MainWindow.Size = UDim2.new(0, VisualConfig.GuiWidth, 0, VisualConfig.GuiHeight)
        MainWindow.Position = UDim2.new(0.5, -VisualConfig.GuiWidth/2, 0.5, -VisualConfig.GuiHeight/2)
    end
end
...
MakeDraggable(floatBtn, BtnContainer)
...
floatBtn.MouseButton1Click:Connect(function()
    if VisualConfig.GestureMode == "Classic" then ToggleGUI() end
end)
