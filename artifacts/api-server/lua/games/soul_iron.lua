...
-- Menggunakan batas waktu (timeout) 5 detik agar script tidak hang saat dijalankan di dalam arena dungeon
local WorldPlaceRE; pcall(function()
    WorldPlaceRE = Services.ReplicatedStorage:WaitForChild("Framework", 5):WaitForChild("Gameplay", 5):WaitForChild("WorldPlace", 5):WaitForChild("WorldUtil", 5):WaitForChild("RemoteEvent", 5)
end)
local GameMatchRE; pcall(function()
    GameMatchRE = Services.ReplicatedStorage:WaitForChild("Remotes", 5):WaitForChild("GameMatchRE", 5)
end)
...
local function GetPositionCFrame(targetPos, posMode)
    ...
    -- [FACE TARGET] Menghadap target secara 3D penuh agar karakter condong/miring mengikuti target
...
local function FireTargetSkills()
    if not EngineConfig.SkillOnTarget then return end
    local now=tick()
    -- Membatasi jeda pengeluaran skill berdasarkan pengaturan cooldown delay
...
local LANG_STRINGS = {
    Indonesia = {
        ...
        lblSkillOnTarget="🎯 Aktifkan Auto Skill",
        ...
        lblBgFx="🌟 Efek Latar",
        secSellByRarity="Jual Berdasarkan Kelangkaan ",
        btnScanSellRarity="Jual", 
        lblSellRarityMinp="Kelangkaan ",
        lblAutoSellRarity="Jual Otomatis Berdasarkan Kelangkaan",
       
       lblRaceReroll="Auto Target Race", 
btnRaceManual="Reroll 1x", 
secRaceReroll="Race Reroll", 
btnRedeemAll="Redem All Code", 
secRedeem="Redem Code", 
lblRaceTarget="Race",
lblRaceSlot = "Pilih Slot Race",
    ...
    English = {
      ...
      tabRoom="🗺️ Room",         tabForge="⚒️ Forge",
        ...
        lblKillAura="⚡ Kill Aura",
        lblSkillOnTarget="🎯 Enable Auto Skill",
        ...
        secSellByRarity="Sell By Rarity",
        lblSellRarityMinp="Rarity",
        lblAutoSellRarity="Auto Sell By Rarity",
        btnScanSellRarity="Sell",
...
Title.RichText = true
Title.Text = 'XIFIL <font color="#ffffff">HUB</font> <font size="10" color="#606080">// IRON SOUL V6</font>'
...
-- [SKILL ON TARGET] Otomatis mengeluarkan skill saat mengunci semua jenis target (Chest/Egg/Monster)
_G.SkillOnTargetToggle = CreateToggleUI(MainFarmPage, "🎯 Auto Skill on Target", EngineConfig.SkillOnTarget, function(v)
    EngineConfig.SkillOnTarget = v
    if v then CustomNotify("🎯 SKILL ON TARGET", "Auto Skill on Target AKTIF!", 2) end
...
--------------------------------------------------------------------------------
do
    local BonusPage = CreateTab("🎁 Bonus", "tabBonus")
    ...
    CreateSection(BonusPage, "Auto Redeem Code", "secRedeem")
    ...
    -- Dropdown pemilihan slot Race (1 sampai 6)
    _G.RaceSlotDropdown = CreateDropdownUI(BonusPage, "Pilih Slot Race", {"1", "2", "3", "4", "5", "6"}, EngineConfig.RaceRerollSlot, function(val)
        EngineConfig.RaceRerollSlot = tostring(val)
    end, "lblRaceSlot")

    -- Tombol Reroll Manual menggunakan slot dinamis yang dipilih
    CreateButton(BonusPage, "🎲 Reroll Race (Manual 1x)", function()
        if RaceRE then 
            pcall(function() RaceRE:FireServer("Rolling", EngineConfig.RaceRerollSlot) end)
        else 
            CustomNotify("RACE", "Remote RaceRE tidak ditemukan.", 3) 
        end
    end, "btnRaceManual")

    _G.RaceRerollToggle = CreateToggleUI(BonusPage, "🔁 Auto Reroll sampai Race Terkunci", EngineConfig.RaceRerollActive, function(v)
        ...
        if v then CustomNotify("RACE", "Auto Reroll AKTIF → target: " .. EngineConfig.RaceLockTarget .. " (Slot: " .. EngineConfig.RaceRerollSlot .. ")", 3) end
    end, "lblRaceReroll")
    ...
    -- Sinkronisasi UI saat config di-load
    _G.SyncBonusUI = function()
        ...
        if _G.RaceSlotDropdown then
            _G.RaceSlotDropdown:SetValue(EngineConfig.RaceRerollSlot)
        end
    end
    end
