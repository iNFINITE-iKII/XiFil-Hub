...
export const data = new SlashCommandBuilder()
  .setName("userkey")
  .setDescription("Cek informasi key milik user, atau cek pemilik sebuah key (Admin only)")
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (targetUser) {
    ...
    for (const uk of userKeys.slice(0, 10)) {
      ...
      fields.push({
        name: `\`${censorKey(uk.license_key)}\``,
        value: `${statusEmoji(license.status)} **${license.status}** • ${durationLabel(license.duration_type, license.duration_value)} • Exp: ${expiryText}`,
        inline: false,
      });
    }
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setColor(0x2196f3)
          .setTitle(`🔑 Keys milik ${targetUser.username}`)
          .setDescription(`<@${targetUser.id}> memiliki **${userKeys.length}** key terdaftar.`)
          .addFields(fields)
          .setFooter({ text: `License Manager • Menampilkan maks 10 key` })
          .setTimestamp(),
      ],
    });
    return;
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: "Max Reset HWID", value: license.max_hwid_resets === -1 ? "Unlimited" : `${license.hwid_reset_count}/${license.max_hwid_resets}`, inline: true },
