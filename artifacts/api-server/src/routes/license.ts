...
> {
  ...
  if (license.expires_at !== null && now > license.expires_at) {
    ...
    await logKeyExpired(key).catch(() => null);
  ...
  if (license.status === "UNUSED") {
    ...
    await logKeyActivated(key, hwid).catch(() => null);
  ...
  if (license.status === "ACTIVE") {
    if (license.max_hwid_count > 1) {
      ...
      await logHwidMismatch(key).catch(() => null);
      return {
        ok: false,
        status: 403,
        message: `Batas akun tercapai. Key ini hanya boleh dipakai di ${license.max_hwid_count} akun Roblox berbeda.`,
        code: "HWID_LIMIT",
      };
    }

    // Single-account mode — allow rebind if HWID was reset
    ...
    if (license.hwid_hash !== hwid) {
      ...
      return {
        ok: false,
        status: 403,
        message: "Akun Roblox tidak cocok. Key ini sudah terikat ke akun Roblox lain.",
        code: "HWID_MISMATCH",
      };
    }
    return {
...
router.get("/check", limiter, async (req, res) => {
  try {
    ...
    res.status(500).json({ status: "error", message: "Server error.", code: "INTERNAL_ERROR" });
  }
});
